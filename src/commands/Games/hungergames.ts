...
import { cleanMentions } from '#utils/utils';
...
export class UserCommand extends CardinalCommand {
	...
	public async messageRun(message: GuildMessage, args: CardinalCommand.Args, context: CardinalCommand.Context) {
		...
		if (filtered.size !== tributes.length) this.error('A user cannot play twice');
		if (this.playing.has(message.channel.id)) this.error('There is already a game in progress in this channel');
		if (filtered.size < 4 || filtered.size > 48) this.error(`I am sorry but the amount of players is less than 4 or greater than 48.`);
		...
		try {
			while (game.tributes.size > 1) {
				...
				const events = game.bloodbath ? hgMessages.hgBloodBath : game.sun ? hgMessages.hgDay : hgMessages.hgNight;

				// Main logic of the game
				const { results, deaths } = this.makeResultEvents(game, events.map(HungerGamesUsage.create));
			...
			const content = `And the winner is... ${game.tributes.values().next().value as string}!`;
	...
	private buildTexts(game: HungerGamesGame, results: string[], deaths: string[]) {
		const headerKey = game.bloodbath ? `Bloodbath` : game.sun ? `Day ${game.turn}` : `Night ${game.turn}`;

		const header = headerKey;
		const death = deaths.length
			? `**${deaths.length}** cannon ${deaths.length > 1 ? 'shots' : 'shot'} can be heard in the distance\n\n${deaths
					.map((d) => `- ${d}`)
					.join('\n')}`
			: '';
