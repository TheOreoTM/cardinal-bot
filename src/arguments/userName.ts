...
import { Argument, type ArgumentContext, Identifiers } from '@sapphire/framework';
...
export class UserArgument extends Argument<User> {
	...
	public async run(parameter: string, context: ArgumentContext<User>) {
		...
		return this.error({
			parameter,
			identifier: `I could not resolve \`${parameter}\` to a user, please make sure you typed their ID correctly!\n\n> **Tip**: You can also use mentions`,
			context
		});
	}
