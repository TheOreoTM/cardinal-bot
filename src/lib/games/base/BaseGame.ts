...
export abstract class BaseGame<T> {
  ...
  protected nextPlayer() {
    this.turn =
      this.turn === GameTurn.PlayerA ? GameTurn.PlayerB : GameTurn.PlayerA;
  }
  ...
  protected onStart(): Promise<unknown> {
    return this.message.edit({
      content: this.render(GameStatus.Start),
      embeds: [],
      attachments: [],
    });
  }
  ...
  protected onUpdate(): Promise<unknown> {
    return this.message.edit({
      content: this.render(GameStatus.Update),
      embeds: [],
      attachments: [],
    });
  }
  ...
  protected onEnd(): Promise<unknown> {
    return this.message.edit({
      content: this.render(GameStatus.End),
      embeds: [],
      attachments: [],
    });
  }
